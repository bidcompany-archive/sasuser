...
%macro new_row(row,plus,minus);
  SELECT	INSERT_TS, SK_ID_NOCFL, REFERENCE_DT, SCENARIO_CD, SUBSCENARIO_CD,
      ...
      case 
        when TEMPLATE_ROW_CD in (&plus) then NSFR_AMT
        when TEMPLATE_ROW_CD in (&minus) then NSFR_AMT*(-1) 
        else 0
      end NSFR_AMT, 
      NSFR_CLUSTER_CD,
		...
		TOT_REC_RVRE, UNENC_SCAD_0_6, UNENC_SCAD_6_12, UNENC_SCAD_OV,TEMPLATE_ROW_CD TEMPLATE_ROW_CD_INPUT
/*
,SEC_CAT_B3_CAT_TYPE
,SEC_CAT_B3_EXP_TYPE
,SEC_CAT_DA_CAT_CD
...
execute(replace view &tdStagedb..NSFR_row_013 as
         (%new_row(%quote('013'),%quote('014','015','016'),%quote('Bazinga'))
         )
        ) by teradata;

execute(replace view &tdStagedb..NSFR_row_020 as
         (%new_row(%quote('020'),%quote('021','022','023'),%quote('Bazinga'))
         )
        ) by teradata;

execute(replace view &tdStagedb..NSFR_row_025 as
         (%new_row(%quote('025'),%quote('026','027','028'),%quote('Bazinga'))
         )
        ) by teradata;

execute(replace view &tdStagedb..NSFR_row_032 as
         (%new_row(%quote('032'),%quote('033','034','035'),%quote('Bazinga'))
         )
        ) by teradata;

execute(replace view &tdStagedb..NSFR_row_060 as
         (%new_row(%quote('060'),%quote('061','062','063'),%quote('Bazinga'))
         )
        ) by teradata;

execute(replace view &tdStagedb..NSFR_row_059 as
         (%new_row(%quote('059'),%quote('049','219'),%quote('054','213'))
         )
        ) by teradata;

execute(replace view &tdStagedb..NSFR_row_230 as
         (%new_row(%quote('230'),%quote('054','213'),%quote('049','219'))
         )
        ) by teradata;

execute(replace view &tdStagedb..NSFR_row_231 as
         (%new_row(%quote('231'),%quote('049'),%quote('Bazinga'))
         )
        ) by teradata;

execute(replace view &tdStagedb..NSFR_row_232 as
         (%new_row(%quote('232'),%quote('234','235','236'),%quote('Bazinga'))
         )
        ) by teradata;

execute(replace view &tdStagedb..NSFR_row_246 as
         (%new_row(%quote('246'),%quote('234','235','236','245'),%quote('237'))
         )
        ) by teradata;
...
FROM	&tdStagedb..NSFR_SHOULDER as b1  
        left join (Select * from  &tdStagedb..NSFR_row_013  
                   ...
                   union all
                   select a1.*, cast('' as varchar(20)) as TEMPLATE_ROW_CD_INPUT FROM P_KGU_GLM_UES.GLRT_WTE_NSFR a1
                  ) b2
      on (
